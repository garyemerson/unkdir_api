...
pub(crate) fn locations() -> Result<(i32, Vec<u8>, &'static str), (i32, String)> {
    let locations = locations_start_end_helper(Utc.ymd(1970, 1, 1).and_hms(0, 0, 0), Utc.ymd(3000, 1, 1).and_hms(0, 0, 0))
...
pub(crate) fn visits() -> Result<(i32, Vec<u8>, &'static str), (i32, String)> {
    let visits = visits_start_end_helper(Utc.ymd(1970, 1, 1).and_hms(0, 0, 0), Utc.ymd(3000, 1, 1).and_hms(0, 0, 0))
        .map_err(|e| (500, format!("Error getting visit data: {}", e)))?;
    let json_str = json::stringify(
            visits.into_iter()
                .map(|v| (&v).into())
...
// ($1, $2, ..., $9),
// ($10, $11, ..., $18)
pub(crate) fn get_param_placeholders(rows: usize, cols: usize) -> String {
    (0..rows)
        .map(|i| {
